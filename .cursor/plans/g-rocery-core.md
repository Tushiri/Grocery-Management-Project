  ...
  **TDD + 100% coverage policy (all phases):** every deliverable follows Red → Green → Refactor. Merge is blocked unless `web/` reports **100%** Vitest coverage (`pnpm test`) and `ai-service/` reports **100%** pytest-cov coverage on `app/` (`./scripts/run-tests.sh`) for any changed service. Full policy: [`docs/testing/tdd-and-coverage.md`](../../docs/testing/tdd-and-coverage.md).
  ...
  - **Status**: **complete** — login + signup wired (`signInWithPassword` / `signUp` + `app/api/household/bootstrap/route.ts` §4.9/§6); `(dashboard)/layout.tsx` server-side auth guard redirects unauthenticated users to `/login`; Vitest suite with **100%** coverage gate on `app/`, `lib/`, `middleware.ts`.
  - **Verify**: new signup produces exactly one `households` row and one `OWNER` membership; session persists across page refresh; unauthenticated access to `(dashboard)` routes redirects to `/login`; `cd web && pnpm test && pnpm type-check` passes.

  ### Phase 3 — Inventory & To-Buy CRUD + Realtime
  - CRUD pages for `inventory_items` and `to_buy_list`; Realtime subscription wired.
  - **Status**: **complete** — inventory and to-buy pages with server actions, auto to-buy for HIGH/MEDIUM items at/below threshold, Realtime hooks filtered by `household_id`; database types cover migrations `0001`–`0008`; **93** Vitest tests at **100%** coverage.
  - **Verify**: RLS blocks cross-household reads (manual test with two accounts); a change made in one browser tab reflects in another tab within ~1s without manual refresh; `cd web && pnpm test && pnpm type-check` passes.
  ...
  - **Status**: **complete** — Cloud Run-ready `Dockerfile` (uv multi-stage, port 8000); `POST /api/process-receipt` wired; clients, services, and schemas in place; **31** pytest tests at **100%** coverage on `app/`.
  ...
  - **Verify**: `uv run uvicorn app.main:app --reload`; `curl localhost:8000/health` → `200`; `POST /api/process-receipt` with no token → `401`; with valid `X-Service-Token` → `200`; `cd ai-service && ./scripts/run-tests.sh` passes with **100%** `app/` coverage; `docker build -t ai-service ./ai-service`.
  ...
  - **Status**: **complete** — Vision OCR line extraction, household-scoped lookup-first pipeline, unmapped lines skipped (Gemini deferred to Phase 6); zero LLM calls when all lines match `product_mapping`.
  - **Verify**: mocked Vision response returns expected lines; pre-mapped strings resolve without Gemini (zero external LLM calls); `normalize_ocr_string("  Org Milk  ")` and `normalize_ocr_string("ORG   MILK")` both equal `"ORG MILK"`; `./scripts/run-tests.sh` passes at **100%** coverage.

  ### Phase 6 — Gemini Structured Extraction Fallback
  - Gemini 2.5 Flash call bound to the strict `GeminiLineExtraction` Pydantic schema for unmapped lines; new mapping persisted after resolution.
  - **Status**: **complete** — unmapped OCR lines route to `GeminiService.extract_structured()` → `resolve_or_create_item` → `persist_mapping`; `matched_via_gemini_count` incremented; re-processing identical raw strings resolves via lookup only (no second Gemini call).
  - **Verify**: unmapped line produces Pydantic-validated extraction (no manual JSON parsing); `persist_mapping` stores normalized `raw_ocr_string`; `./scripts/run-tests.sh` passes at **100%** coverage.

  ### Phase 7 — Reconciliation Engine (Partial Fulfillment)
  - `/approve` and `/reject` endpoints: inventory increment, `price_history` insert, `to_buy_list` decrement.
  - **Status**: **complete** — `apply_line_item` implements partial to-buy fulfillment (`OPEN` → `PARTIAL` → `FULFILLED`); approve/reject protected by `X-Service-Token`.
  - **Verify** (core acceptance test from the brief): seed `to_buy_list` with `quantity_requested=3, quantity_remaining=3, status=OPEN`; approve a receipt line with `quantity=1` for the same item → assert `quantity_remaining=2`, `status=PARTIAL`, row **not deleted**; approve remaining `quantity=2` → `quantity_remaining=0`, `status=FULFILLED`; `./scripts/run-tests.sh` passes at **100%** coverage.

  ### Phase 8 — End-to-End Receipt Upload UI
  - Upload form → Storage → route handler → FastAPI `process-receipt` → review UI → approve.
  - **Status**: **complete** — `ReceiptUploadForm` uploads to Storage; `POST /api/receipts` inserts `pending_receipt` and proxies to FastAPI; `/receipts/[id]` review UI edits line items and approves via `POST /api/receipts/[id]/approve`.
  - **Verify**: parsed line items render for review with editable quantity/prices; approval proxies to reconciliation engine; `pnpm test` passes at **100%** coverage.
  ...
  - **Status**: **complete** (build prep) — `@ducanh2912/next-pwa`, `manifest.json`, icons, and production build config in place; `pnpm build` succeeds with `productionBrowserSourceMaps: false`.
  - **Verify**: production build compiles cleanly; no embedded secret values in client bundles; manual deploy to Vercel + Cloud Run and Lighthouse PWA audit remain as deployment follow-ups.
